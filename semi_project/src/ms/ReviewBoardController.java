...
public class ReviewBoardController extends HttpServlet {
	 ...
	 * request.getParameter("email"); UsersDao udao = UsersDao.getInstance();
	 * UsersVo uvo = udao.select(email); String title =
	 * request.getParameter("title"); String content =
	 * request.getParameter("content"); int height =
	 * Integer.parseInt(request.getParameter("height")); int weight =
	 * Integer.parseInt(request.getParameter("weight")); String img =
	 * request.getParameter("img"); System.out.println(email + title + content +
	 * height + weight + img); ReviewBoardVo vo = new ReviewBoardVo(0,
	 * uvo.getName(), email, title, content, height, weight, 0, null, img);
	 * ReviewBoardDao dao = ReviewBoardDao.getInstance(); int n = dao.insert(vo); if
	 * (n > 0) { list(request, response); } }
	 */
	...
	public void searchlist(HttpServletRequest request, HttpServletResponse response)
		...
		int height = 0;
		int weight = 0;
		if (Integer.parseInt(request.getParameter("height")) != 0
...
//			}
	...
	public void update(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isMulti) {
			...
			ReviewBoardVo vo = new ReviewBoardVo(num, vo1.getName(), vo1.getEmail(), title, content, vo1.getHeight(),
					vo1.getWeight(), vo1.getHit(), vo1.getRegdate(), img, vo1.getItemImg(), vo1.getCode());
			int n = dao.update(vo);
			if (n > 0) {
	 ...
	 * throws ServletException, IOException { ReviewBoardDao dao =
	 * ReviewBoardDao.getInstance(); int num =
	 * Integer.parseInt(request.getParameter("num")); ReviewBoardVo vo1 =
	 * dao.detail(num); String title = request.getParameter("title"); String content
	 * = request.getParameter("content"); String img = request.getParameter("img");
	 * System.out.println(num + " " + title + " " + content + " " + img);
	 * ReviewBoardVo vo = new ReviewBoardVo(num, vo1.getName(), vo1.getEmail(),
	 * title, content, vo1.getHeight(), vo1.getWeight(), vo1.getHit(),
	 * vo1.getRegdate(), img, vo1.getItemImg(), vo1.getCode()); int n =
	 * dao.update(vo); if(n>0) { list(request, response); } }
	 */
