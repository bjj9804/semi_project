		...
		<div class="inner">
			...
			<c:choose>
				<c:when test="${startPage>10 }">
					<a
						href="qnalist.do?cmd=list&pageNum=${startPage-1 }&email=${email}"><span
						style="color: black">[이전]</span></a>
				</c:when>
				<c:otherwise>
			...
			<c:forEach var="i" begin="${startPage }" end="${endPage }">
				<c:choose>
					<c:when test="${pageNum==i }">
						<a href="qnalist.do?cmd=list&pageNum=${i }&email=${email}"><span
			...
			<c:choose>
				<c:when test="${endPage<pageCount }">
					<a href="qnalist.do?cmd=list&pageNum=${endPage+1 }&email=${email}"><span
						style="color: black">[다음]</span></a>
				</c:when>
				<c:otherwise>
